    ...
    <div class="min-h-screen bg-black pt-32 pb-16 px-4 sm:px-6 lg:px-8 relative overflow-hidden flex items-center justify-center">
      ...
      <div class="relative z-10 w-full max-w-md bg-[rgba(20,12,30,0.55)] backdrop-blur-md border border-purple-500/20 rounded-2xl p-8 shadow-2xl animate-fade-in-up">
        ...
        <div class="text-center mb-8">
            ...
            <p class="text-gray-400">Únete a Unión Pegaso y transforma tu negocio</p>
        </div>
        ...
        <form [formGroup]="registerForm" (ngSubmit)="onSubmit()" class="space-y-4">
           ...
           <div>
             ...
             @if (registerForm.get('password')?.touched && registerForm.get('password')?.invalid) {
                <div class="mt-1 text-xs text-red-400 space-y-1">
                    ...
                    <p *ngIf="registerForm.get('password')?.errors?.['minlength']">Mínimo 8 caracteres.</p>
                    <p *ngIf="registerForm.get('password')?.errors?.['pattern']">Requiere mayúscula, minúscula, número y carácter especial.</p>
                </div>
             }
           </div>
           ...
           <button 
             ...
             class="w-full py-3 px-4 bg-purple-600 hover:bg-purple-700 text-white font-semibold rounded-lg shadow-[0_0_15px_rgba(147,51,234,0.3)] hover:shadow-[0_0_25px_rgba(147,51,234,0.5)] transition-all disabled:opacity-50 disabled:cursor-not-allowed flex justify-center items-center mt-6"
           >
             @if (loading()) {
        ...
        <p class="mt-8 text-center text-sm text-gray-400">
            ¿Ya tienes cuenta? 
            <a routerLink="/login" class="text-purple-400 hover:text-purple-300 font-medium transition-colors">Inicia sesión</a>
        </p>
      </div>
    </div>
  `
    ...
    if (this.registerForm.valid) {
      ...
      this.authService.register({ nombre: nombre!, email: email!, password: password! }).subscribe({
        ...
        error: (err) => {
          ...
          this.errorMessage.set(err.error?.error || 'Error al registrarse. Inténtelo de nuevo.');
        }
      });
    } else {
